...
corTable = abs(cor(dogtrain_processed, y = dogtrain_processed$dogTrainD...1.))
head(corTable, 6)
...
dogTrainD$target <- as.factor(dogTrainD$target)
dogTestD$target <- as.factor(dogTestD$tar)

dogTrainD <- na.omit(dogTrainD)
dogTestD <- na.omit(dogTestD)

z_scores <- apply(dogTrainD[,-1], 2, function(x) abs(scale(x)))
dogTrainD <- dogTrainD[rowSums(z_scores < 3) == ncol(z_scores),]

scaled_train <- scale(dogTrainD[,-1])
scaled_test <- scale(dogTestD[,-1])

dogtrain_processed <- data.frame(dogTrainD[,1], scaled_train)
dogtest_processed <- data.frame(dogTestD[,1], scaled_test)


hist(dogtrain_processed[,1])

boxplot(dogtrain_processed[,2:513])

barplot(table(dogtrain_processed[,1]))
barplot(table(dogtrain[,2]))

corr_matrix <- cor(dogtrain_processed[,2:513])
heatmap(corr_matrix)
